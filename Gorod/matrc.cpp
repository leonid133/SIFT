...
//Умножение A = A * Filter и сверта в temp
...
matric<float> MoveOxy(image &rimg, image &rimg2, int ColorFlag)
{
   ...
   X1 = (okresnost+R+alfa); //if((float)r.n/2-200>X1){X1 = (int)(float)r.n/2-200;}   //здесь можно ограничить зону исследования начальным перекрытием
   Y1 = (okresnost+R+alfa); //if((float)r.m/2-300>Y1){Y1 = (int)(float)r.m/2-300;}   //здесь можно ограничить зону исследования начальным перекрытием
   X2 = rimg.Y.n-(okresnost+R+alfa); //if((float)r.n/2+200<X2){X2 = (int)(float)r.n/2+200;}  //здесь можно ограничить зону исследования начальным перекрытием
   Y2 = rimg.Y.m-(okresnost+R+alfa); //if((float)r.m/2+300<Y2){Y2 = (int)(float)r.m/2+300;}  //здесь можно ограничить зону исследования начальным перекрытием
